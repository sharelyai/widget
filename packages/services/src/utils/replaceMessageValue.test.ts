...
describe('replaceMessageValue', () => {
  ...
  it('removes AI_TOOL_MARKER', () => {
    expect(replaceMessageValue({ message: 'before AI_TOOL_MARKER after' })).toBe('before  after');
  });

  it('removes !API! loading markers', () => {
    expect(replaceMessageValue({ message: 'loading !API! done' })).toBe('loading  done');
  });

  it('removes [source] without parentheses', () => {
    expect(replaceMessageValue({ message: 'text [source] more' })).toBe('text  more');
  });
