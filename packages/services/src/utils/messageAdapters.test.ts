...
describe("agentMessageToBodyMessage", () => {
  it("converts user role to USER type", () => {
    const result = agentMessageToBodyMessage(
      makeAgentMessage({ role: "user" }),
    );
  ...
  it("converts assistant role to AI type", () => {
    const result = agentMessageToBodyMessage(
      makeAgentMessage({ role: "assistant" }),
    );
  ...
  it("builds sourcesMetadata keyed by index in sources[]", () => {
    ...
    expect(result.sourcesMetadata![0].metadata).toMatchObject({
      source: "0",
      title: "First",
    });
    expect(result.sourcesMetadata![1].source).toBe("1");
    expect(result.sourcesMetadata![1].metadata).toMatchObject({
      source: "1",
      title: "Second",
    });
  });
  ...
  it("rewrites [N] citation markers to markdown links with the index as href", () => {
    ...
    expect(result.message).toBe(
      "See [First](0) and also [Third](2) for context.",
    );
  });
